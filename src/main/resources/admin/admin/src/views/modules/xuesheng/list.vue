<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('xuesheng','统计考勤') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="kaoqintongjiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'统计考勤':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','统计考勤') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="kaoqintongjiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'统计考勤':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','统计考勤') && contents.tableBtnIcon == 0" type="success" size="mini" @click="kaoqintongjiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'统计考勤':'' }}</el-button>
    ...
    <kaoqintongji-cross-add-or-update v-if="kaoqintongjiCrossAddOrUpdateFlag" :parent="this" ref="kaoqintongjiCrossaddOrUpdate"></kaoqintongji-cross-add-or-update>
...
import kaoqintongjiCrossAddOrUpdate from "../kaoqintongji/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      kaoqintongjiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    kaoqintongjiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.kaoqintongjiCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.kaoqintongjiCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.kaoqintongjiCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
    },
